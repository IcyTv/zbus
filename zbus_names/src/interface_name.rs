...
impl InterfaceName<'static> {
    /// Create a new static interface name, validating it at compile time in const contexts.
    ///
    /// # Panics
    ///
    /// Panics if `name` is not a valid D-Bus interface name.
...
const fn validate_bytes_const(bytes: &[u8]) -> bool {
